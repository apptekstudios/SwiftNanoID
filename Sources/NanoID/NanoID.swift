...
public final class NanoID: Identifiable, Hashable, Equatable, Codable, Sendable {
    ...
    /// Controls whether validation is enforced during decoding.
    ///
    /// When `true` (the default), decoding a NanoID from JSON or other formats
    /// will validate that the string conforms to the configured ``length`` and
    /// ``alphabet``. Invalid strings will cause decoding to fail with an error.
    ///
    /// When `false`, decoding will accept any string without validation. This is
    /// useful when working with NanoIDs generated by other systems that may use
    /// different configurations, or when migrating data.
    ///
    /// - Important: This setting only affects ``init(from:)``. The throwing
    ///   initializer ``init(_:)`` always validates regardless of this setting.
    ///
    /// - Warning: Disabling strict mode may allow invalid data into your system.
    ///   Use with caution and only when necessary.
    ///
    /// ## Example
    ///
    /// ```swift
    /// // Disable validation for decoding legacy data
    /// NanoID.strict = false
    ///
    /// let json = #"{"id":"any-string-works-now"}"#
    ...
    /// print(item.id.string) // "any-string-works-now"
    ///
    ...
    ///
    /// Used internally by `from(unchecked:)` and non-strict decoding.
    ...
    /// Creates a NanoID from a string without performing any validation.
    ///
    /// Use this method when you have a string that you know is a valid NanoID
    /// but may not conform to the current ``length`` or ``alphabet`` configuration.
    /// This is useful for:
    /// - Working with NanoIDs generated by other systems
    /// - Migrating data from legacy systems
    /// - Performance-critical code paths where validation overhead is unacceptable
    ///
    /// - Parameter string: The string to use as the NanoID value.
    ///
    /// - Returns: A new NanoID instance containing the provided string.
    ///
    /// - Warning: No validation is performed. The caller is responsible for
    ///   ensuring the string is appropriate for use as a NanoID.
    ///
    /// ## Example
    ///
    /// ```swift
    /// // Create from a string without validation
    /// let legacyId = NanoID.from(unchecked: "legacy-id-from-old-system")
    ///
    /// // Works even if string doesn't match current configuration
    /// NanoID.length = 10
    /// let longId = NanoID.from(unchecked: "this-is-much-longer-than-10")
    /// print(longId.string) // "this-is-much-longer-than-10"
    /// ```
    public static func from(unchecked string: String) -> NanoID {
        return NanoID(unchecked: string)
    }
    ...
    ///
    /// The behavior depends on the ``strict`` setting:
    /// - When `strict` is `true` (default): Validates the decoded string against
    ///   ``length`` and ``alphabet``, throwing an error if invalid.
    /// - When `strict` is `false`: Accepts any string without validation.
    ///
    /// - Parameter decoder: The decoder to read data from.
    ///
    /// - Throws: ``Error/invalidIDStringLength`` if ``strict`` is `true` and
    ///   the decoded string length doesn't match the configured ``length``.
    /// - Throws: ``Error/invalidIDStringChars`` if ``strict`` is `true` and
    ///   the decoded string contains invalid characters.
